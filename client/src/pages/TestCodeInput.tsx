...
const TestCodeInput: React.FC = () => {
  ...
  return (
    <div>
      {
        result ? (
          <div className="flex justify-center items-start py-10 mb-30 ">
            <form onSubmit={onSubmit} className="flex flex-col">
              <h1 className="text-4xl text-black text-center  mb-1200 ">
                Enter Test
              </h1>
              <p className="text-2xl py-3">Enter test code</p>
              <input
                className="placeholder:italic placeholder:text-slate-400 block bg-white mb-4 w-full border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm"
              ...
              <button
                type="submit"
                className="px-6 py-2 mt-4 rounded-md mx-auto active:bg-blue-400 text-[#fdd017]  bg-black"
              >
                Submit
              </button>
            </form>
          ...
          <div className="p-8 flex flex-col gap-4">
            ...
            <div>
              {test?.status === "completed" && (
                <Link to={`/student/${username}/test/${testCode}/result`}>
                  Show result
                </Link>
              )}
              {test?.status === "upcoming" && (
                <h2>Wait, Test is about to start</h2>
              )}
              {test?.status === "ongoing" && (
                <button
                  className="bg-green-600 px-4 py-2 text-white"
                  onClick={() =>
                    navigate(`/student/${username}/test/${testCode}`)
                  }
                >
                  Start
                </button>
