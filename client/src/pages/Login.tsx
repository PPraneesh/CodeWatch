  ...
  const handleFormSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      ...
      if (res.data.message === "User exists") {
        ...
        res.data.message === "User created and logged in"
      ) {
  ...
  return (
    <div className="w-full min-h-screen flex flex-row bg-gray-200">
      ...
      <div className="relative w-7/12">
        ...
        <div className="relative z-10 flex justify-center items-center backdrop-blur-[3px] h-full">
          <h1 className="text-7xl text-white ">
            Code <span className="text-[#f8b739]">Watch</span>
          </h1>
        </div>
      </div>

      {/* right */}
      <div className="w-5/12 px-20 my-auto">
        <p className="text-2xl text-black text-center">
          {login ? "Welcome Back!" : "Create a New Account"}
        </p>
        <h1 className="text-3xl font-semibold text-black text-center">
          {" "}
          {login ? "Login to your account" : "Register"}{" "}
        </h1>

        <form
          onSubmit={handleSubmit(handleFormSubmit)}
          className="mt-8 px-8 py-10 bg-black rounded-lg flex flex-col gap-4"
        >
          {!login && (
            <div>
          ...
          <div className="w-full">
            ...
            <div className="flex flex-row gap-4">
              <div
                className={`w-1/2 px-4 py-2 rounded-lg text-center cursor-pointer ${
                  userType === "student"
                    ? "bg-[#f8b739] "
          ...
          <button
            type="submit"
            className="px-4 py-2 mt-4 bg-[#f8b739] rounded active:bg-white active:text-black"
          >
